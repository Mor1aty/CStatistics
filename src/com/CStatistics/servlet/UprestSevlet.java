...
@WebServlet("/upRest")
	public class UprestSevlet extends HttpServlet {
		...
		protected void doPost(HttpServletRequest request, HttpServletResponse response)
			...
			// 参数校验
			result = HttpUtil.checkParams(params, new String[] {"time","clazz", "minister", "member1", "member2", "result"});
			// 校验不通过，适用于需要请求参数的情况
		...
		try{
            ...
            int y=dao.uprest(params.get("time"), params.get("clazz"), params.get("minister"), params.get("member1"), params.get("member2"), params.get("result"));
			if(y>0){
				result = JsonUtil.jsonResponse(null, AllConstant.CODE_SUCCESS, "提交成功");
			}else{
